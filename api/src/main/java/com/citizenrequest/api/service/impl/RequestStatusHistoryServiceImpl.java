...
public class RequestStatusHistoryServiceImpl implements RequestStatusHistoryService {
    ...
    @Override
    public List<RequestStatusHistoryDto> findDepartmentRequestHistory(Long requestId, Long employeeId) {
        ...
        if (!request.getDepartment().getId().equals(employee.getDepartment().getId())) {
            throw new RuntimeException("Employee can only view history for requests assigned to their department.");
        }
